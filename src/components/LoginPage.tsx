    ...
    <div className="h-screen w-screen overflow-hidden flex flex-col bg-white">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} autoComplete="off">
              ...
              <Field icon={<Lock className="w-4 h-4" />} label="Password">
                <div className="relative">
                  <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="ENTER PASSWORD" autoComplete="new-password" className="w-full px-4 py-2.5 pr-10 rounded-md border border-border bg-white focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition placeholder:text-muted-foreground/60 text-sm" />
              ...
              <div className="flex items-center gap-2 mb-2">
                <div className="flex-1 border border-border rounded-md py-2.5 grid place-items-center select-none relative overflow-hidden bg-secondary">
                  <span className="text-xl font-bold italic tracking-[0.35em] text-foreground line-through decoration-2 select-none" style={{ fontFamily: "Georgia, serif", textShadow: "1px 1px 0 rgba(0,0,0,0.08)" }}>{captchaCode}</span>
                </div>
                <button type="button" onClick={refreshCaptcha} title="Refresh CAPTCHA" className="p-2.5 rounded-md border border-border hover:bg-secondary text-muted-foreground hover:text-destructive transition"><RefreshCw className="w-4 h-4" /></button>
              </div>
      ...
      {error && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 px-4" onClick={() => setError(null)}>
          <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-xl shadow-2xl border border-border max-w-sm w-full overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-destructive/10">
              <div className="flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-destructive" /><h3 className="font-bold text-foreground text-sm">{error.title}</h3></div>
              <button onClick={() => setError(null)} className="p-1 hover:bg-secondary rounded"><X className="w-4 h-4 text-muted-foreground" /></button>
            </div>
            <div className="px-5 py-5">
              <p className="text-sm text-foreground/80 leading-relaxed">{error.msg}</p>
              <div className="mt-5 flex justify-end"><button onClick={() => setError(null)} className="px-4 py-2 rounded-md bg-destructive text-destructive-foreground text-sm font-semibold hover:brightness-110 transition">OK, Try Again</button></div>
            </div>
          </div>
        </div>
      )}
      ...
      {showSuccess && (
        <div className="fixed bottom-6 right-6 z-50 overflow-hidden rounded-xl flex items-center gap-3 pl-4 pr-5 py-3 min-w-[260px] max-w-xs bg-foreground shadow-2xl">
          <div className="w-8 h-8 rounded-full grid place-items-center shrink-0 bg-success/20"><CheckCircle2 className="w-5 h-5 text-success" /></div>
          <div className="text-sm font-medium text-background tracking-tight">Login successfully</div>
          <div className="absolute bottom-0 left-0 h-[2px] w-full bg-success" />
        </div>
      )}
    </div>
  );
...
function Field({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) {
  return <div className="mb-1"><div className="flex items-center gap-2 text-sm font-medium text-foreground mb-1.5"><span className="text-muted-foreground">{icon}</span>{label}</div>{children}</div>;
}

function FloatingBubbles() {
  return <>{[...Array(8)].map((_, i) => <div key={i} className="absolute rounded-full bg-white/20 backdrop-blur-sm" style={{ width: 40 + i * 20, height: 40 + i * 20, left: `${(i * 13) % 80}%`, top: `${(i * 17) % 70}%` }} />)}</>;
}